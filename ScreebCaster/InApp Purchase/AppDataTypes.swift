...
struct Section {
    ...
    func contains( identifier: String) -> Bool {
        ...
        if
            let elements = elements as? [String],
            elements.contains(identifier) {
                return true
        } else {
            return false
        }
    ...
    func get( product identifier: String) -> SKProduct? {

        if
            let elements = elements as? [SKProduct] {
            return elements.first(where: { (product) -> Bool in
