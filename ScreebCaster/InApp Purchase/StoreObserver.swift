...
class StoreObserver: NSObject {
	...
	*/
	...
	fileprivate func handlePurchased(_ transaction: SKPaymentTransaction) {
        ...
        Session.current().states[transaction.payment.productIdentifier] = .purchased
        Session.current().purchasedOrRestored[transaction.payment.productIdentifier] = true
        Session.archive()

        DispatchQueue.main.async {
            self.delegate?.handlePurchased(transaction)
        }
	...
	fileprivate func handleRestored(_ transaction: SKPaymentTransaction) {
        ...
        Session.current().states[transaction.payment.productIdentifier] = .restored
        Session.current().purchasedOrRestored[transaction.payment.productIdentifier] = true
        Session.archive()

        DispatchQueue.main.async {
            self.delegate?.handleRestored(transaction)
        }
